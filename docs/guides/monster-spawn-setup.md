...
## 4-1. 먼저 할 것 — 최소 검증 (2026-08-09)

**스포너를 다 붙이기 전에 스폰과 토템 연동만 확인한다.**
몬스터 아트·애니메이션이 들어오면 프리팹이 바뀔 수 있어, 그 전에
**컴포넌트 하나로** 두 가지를 함께 본다.

### 붙일 것은 하나뿐이다

`Goblin2F` 루트에 `FloorPopulation` 하나. **1F·3F 스포너는 아직 안 붙인다.**

```
Goblin2F  (GameObject)
 └─ [C] FloorPopulation                     ← 이것만 추가
          Monster Prefab → Monster_Temp     ★ 이미 빨간색이다 (Color 1,0,0,1)
          Entries → 크기 1
              [0] Definition → Monster_goblin_hob · Weight 1
          Min Spawn Distance → 8            ← 검증용으로 좁힌다. 기본 14 는 화면 밖
          Max Spawn Distance → 16
          Spawn Interval     → 1            ← 기본 2.5 는 기다리기 지루하다
          Despawn Distance   → 45
```

2F 를 고르는 이유는 **거기에만 토템이 있기 때문**이다.
`Goblin_2F` 배치 프로필이 메인 1 + 서브 3 을 뿌리고, 각 지분이 4 라
**목표 인구가 16** 으로 시작한다.

### 무엇을 보나

인구를 볼 HUD 가 없으므로 **재생 중 인스펙터에서 `FloorPopulation` 우클릭 → `인구 상태`**.
...
| 단계 | 기대 |
|---|---|
| 1. 2F 진입 후 가만히 있는다 | 1초마다 한 마리씩 늘어 **16에서 멈춘다** |
| 2. `인구 상태` | 출처가 **"토템 지분 합 16"** — 인스펙터 값이 아니어야 한다 |
| 3. **서브 토템** 하나를 부순다 | 목표가 **12** 로 준다. **살아 있는 것은 그대로다** |
| 4. **메인 토템**을 부순다 | 출처가 **"메인 토템이 부서져 0"**. 더 안 채워진다 |
| 5. 남은 몬스터를 잡는다 | 다시 안 나온다. 경험치는 오른다 |
| 6. 1F 로 갔다가 2F 로 돌아온다 | **여전히 0이어야 한다** ← 아래 |

### ⚠ 6번이 핵심이다

**토템은 부수는 순간 몬스터를 없애지 않는다.** 줄어드는 것은 목표치뿐이고
실제 수는 잡히면서 수렴한다 — 부수는 순간 증발하면 그 행동이 공짜가 되어
"지금 부술까, 정리하고 부술까" 라는 판단이 사라진다
(`docs/design/map-objects.md` §1).

그래서 눈으로 보이는 신호는 **"사라진다" 가 아니라 "다시 안 나온다"** 이다.

6번은 그 약속이 **층을 나갔다 와도 지켜지는지**를 본다.
`FloorProps` 가 세이브에서 토템을 부서진 채로 복원하는데, 그 복원은
`SpawnTotem.Start` 보다 먼저 일어난다. 지분을 조건 없이 얹으면
**부숴 둔 토템이 지분을 되살려 몬스터가 다시 차오른다** (2026-08-09 수정).
...
| 2F 에서 한두 마리만 나오고 만다 | 토템이 목표를 정한다. `인구 상태` 로 출처를 본다 |
| 토템을 부쉈는데 몬스터가 안 사라진다 | **의도된 것.** 목표만 줄고 실제 수는 잡히면서 수렴한다 (4-1절) |
| 층을 나갔다 오니 다시 차오른다 | 2026-08-09 이전 버그. `SpawnTotem` 이 부서진 토템의 지분을 되살렸다 |
