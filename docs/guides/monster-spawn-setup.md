...
| 히든 방 안에 스폰된다 | `NoSpawnZone` 이 없다. 지금 씬에 0개다 — 아래 참조 |
...
- **`NoSpawnZone` 이 씬에 0개다.** 히든 방 안에 몬스터가 뿌려질 수 있다.
  들어가면 오브젝트를 부술 수 없어 영영 갇히고, 그대로 층 인구 상한을 차지해
  토템 지분 모델이 샌다. 히든 방마다 하나씩 두어야 한다
