...
`경험치 +100` 을 한 번 누른 뒤의 기대 출력:

```
[GameRoot] 현재 상태 — 신규 플레이
  Lv 2  EXP 75/76  HP 120/120
  스탯 : VIT 24 / ATK 12 / DEF 6 / AGI 9 / FOC 6
  세이브 : C:\Users\...\Pretty_Knights\save.json  (존재 False)
```

레벨 1→2 에 25 가 필요하므로 한 번 오르고 75 가 남는다.
2→3 은 `25 × 2^1.6 ≈ 76` 이라 한 점 모자라 멈춘다.

**HP 최대치가 100 → 120 으로 늘고 가득 찬 상태가 되면**
정의 → 런타임 계산 → 상태 갱신이 전 구간 연결된 것이다.
(`AddExp` 는 레벨업 시 HP 를 새 최대치로 채운다)
