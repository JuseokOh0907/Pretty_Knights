...
> **그리는 것은 "막을 영역"이다.** 비어 보이는 곳을 채우는 게 아니라
> 콜라이더가 있어야 할 자리를 칠한다.
> 큰 사각형 하나로 셀 전체를 덮으면 결과가 `Grid` 와 똑같아진다.

1. 모서리 타일 PNG 선택 → 인스펙터의 **Sprite Editor** 버튼
2. 좌상단 드롭다운을 **`Custom Physics Shape`** 로 변경
3. **직각 사각형 두 개를 겹쳐 그린다.** 빈 곳을 드래그할 때마다 경로가 추가된다
4. **Apply**

TL(좌상단) 모서리라면 벽은 위쪽 가로 띠와 왼쪽 세로 띠 두 갈래이고
안쪽 우하단 사분면이 바닥이다.
...
정점 6개짜리 L자 다각형 하나보다 **직각 사각형 두 개가 정확하고 쉽다.**
모서리에서 겹쳐도 `Merge` 가 합쳐주므로 문제없다.

자동 단순화를 거치지 않으므로 삼각형이 나오지 않는다.
...
| 확인 | 내용 |
|---|---|
| `Collider Type` | **`Grid` 면 커스텀 셰이프를 무시하고 칸 전체를 쓴다.** 가장 흔한 원인 |
| Sprite Editor | `Apply` 를 눌렀는지 |
| 재생성 | `TilemapCollider2D` 를 껐다 켜거나 타일 에셋 우클릭 → Reimport. `CompositeCollider2D` 를 쓰면 특히 필요 |
| Scene 뷰 | 초록선이 사각형이면 셰이프 미적용, 삼각형이면 자동 외곽선이 그대로 남은 것 |
