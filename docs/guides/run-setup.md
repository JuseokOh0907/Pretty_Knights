...
10개가 동시에 켜지면 마지막 하나가 이기고 나머지는 덮어쓰기 경고만 남긴다.
어느 것이 이길지는 등록 순서에 달려 있어 **플레이어가 선 층과 다른 층의 판정이 쓰일 수 있다.**
스폰 위치 검사·포탈 도착 보정·순간이동 목적지가 전부 여기를 거치므로 조용히 어긋난다.

> 층들은 월드 좌표가 서로 떨어져 있어 **화면에서 겹쳐 보이지는 않는다**
> (Goblin 1F 는 원점 `(0, 500)`, 2F 는 `(250, 500)`, 3F 는 `(150, 500)`).
> 문제는 그리기가 아니라 **등록이 하나뿐**이라는 데 있다.
...
> **`△ 도착 지점 …` 은 두 가지를 갈라 알려준다.**
> `그 칸에 바닥 타일이 없음` 이면 바닥 안쪽으로 옮기고,
> `벽(Guide) 타일 위에 있음` 이면 벽에서 비켜야 한다. 고치는 방법이 정반대다.
> 바닥이 깔린 자리여도 그 위에 벽 타일이 있으면 설 수 없다.
