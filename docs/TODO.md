...
- [ ] **배치 도구** `Pretty Knights > Props > 오브젝트 뿌리기`
      ...
      - 연결성 검사 2단 — 벽만 보는 **강한 검사는 배치 거부**,
        파괴 가능 오브젝트까지 보는 약한 검사는 경고만
...
**판정 코어는 하나, 스킬 목록은 플레이어와 몬스터로 나눈다** (2026-08-05).
...
SkillShape        범위 계산 한 벌  — Forward / Line / Cross / Area / Dash
   ├ PlayerSkill  플레이어가 배우는 것.  스킬로만 ATK·DEF 가 오른다
   └ MonsterSkill 몬스터 등급별.        보스 패턴 포함
```
